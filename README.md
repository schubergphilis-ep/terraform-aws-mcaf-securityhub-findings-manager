...
* **ProductName Filtering**:  Optionally restrict ticket creation to specific AWS products using either `jira_integration.include_product_names` (allow list) or `jira_integration.exclude_product_names` (deny list). Both default to `[]`, meaning all products, and they are **mutually exclusive** - set at most one of them. For example, `include_product_names = ["Security Hub"]` only creates tickets for Security Hub findings, while `exclude_product_names = ["Inspector"]` creates tickets for every product except Inspector. Filtering is implemented at the Step Function level for optimal performance and applies globally to all Jira instance configurations.
...
| Name | Type |
|------|------|
| [aws_cloudwatch_event_rule.securityhub_findings_deleted_resources](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_rule) | resource |
...
| [aws_cloudwatch_event_rule.securityhub_findings_passed_events](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_rule) | resource |
...
| [aws_cloudwatch_event_target.jira_orchestrator_deleted_resources](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_target) | resource |
| [aws_cloudwatch_event_target.jira_orchestrator_passed](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_target) | resource |
...
| <a name="input_s3_bucket_name"></a> [s3\_bucket\_name](#input\_s3\_bucket\_name) | The name for the S3 bucket which will be created for storing the function's deployment package | `string` | `null` | no |
...
| <a name="output_kms_key_arn"></a> [kms\_key\_arn](#output\_kms\_key\_arn) | The ARN of the KMS key created by this module, which can be used to encrypt the secrets stored in Secrets Manager and SSM Parameter Store used by this module. This will be null when an existing key is provided via 'kms\_key\_arn'. |
<!-- END_TF_DOCS -->
